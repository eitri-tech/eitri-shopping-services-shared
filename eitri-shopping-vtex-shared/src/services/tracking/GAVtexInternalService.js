...
export default class GAVtexInternalService {
  ...
  static _resolveCategory = (item) => {
    ...
    let categories =
      item.productCategories || item.categories[0]?.split("/").filter(Boolean);
    let result = {};
  ...
  static addItemToCart = (addedItems, cart) => {
    try {
      ...
      const products = _addedItems
        .map((addedItem) => cart.items.find((i) => i.id === addedItem.id))
        .filter((i) => !!i);
  ...
  static addShippingInfo = (cart) => {
    ...
    try {
      ...
      let shippingSelected = cart?.shippingData?.logisticsInfo?.map(
        (item) => item.selectedSla,
      );
      const uniqueSla = [...new Set(shippingSelected)];

      const totalItemPrice =
        cart.totalizers.find((item) => item.id === "Items")?.value / 100;
    ...
    try {
      ...
      let paymentSelected = cart?.paymentData?.payments?.map((item) => {
        const paymentSystem = cart?.paymentData?.paymentSystems.find(
          (payment) => payment.stringId === item.paymentSystem,
        );
  ...
  static purchase = (cart, orderId) => {
    try {
      ...
      const totalItemPrice =
        cart.totalizers.find((item) => item.id === "Items")?.value / 100;
      const shippingPrice =
        cart.totalizers.find((item) => item.id === "Shipping")?.value / 100;
