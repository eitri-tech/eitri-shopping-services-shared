...
export default class SalesCartService {
	...
	private static async _getOrCreateAssociateCode(orderFormId: string): Promise<string> {
	...
	private static async _resolveUserContext(
		...
		return SalesUserService.resolveUserContext(vtexIdToken, baseUrl, responseHeaders, fallback)
	}
	...
	private static async _createOrderForm(
		...
		if (ofSetCookies) {
			const cookieString = Array.isArray(ofSetCookies) ? ofSetCookies.join('; ') : String(ofSetCookies)
			const match = cookieString.match(/userContext=([^;,\s]+)/)
			if (match) {
				userContext = decodeURIComponent(match[1])
				console.log('[SalesCartService] userContext extracted from orderForm response')
			}
		}

		if (!userContext) {
			console.warn('[SalesCartService] WARNING - userContext not found, subsequent steps may fail with 401')
		}
	...
	private static async _checkIn(
		...
		console.log('[SalesCartService] Check-in completed with status', checkInRes.status)
	}
	...
	private static async _setShippingAddress(
		...
		console.log('[SalesCartService] Shipping address set with status', res.status)
	}
	...
	private static async _setSession(baseUrl: string, sc: string): Promise<void> {
		...
		console.log('[SalesCartService] Session set with status', sessionRes.status)
	}
	...
	private static async _salesAppLogin(
		...
		const result = await SalesUserService.salesAppLogin(baseUrl, account, vtexIdToken, existingUserContext)
		console.log(
	...
	private static async _associateVendor(
		...
		const gqlHeaders = SalesCartService._graphqlHeaders(vtexIdToken, account, userContext)
		...
		console.log('[SalesCartService] Vendor association completed with status', associateRes.status)
	}

	/**
	 * Sets anonymous customer profile.
	 */
	private static async _setAnonymousProfile(baseUrl: string, orderFormId: string): Promise<void> {
		...
		console.log('[SalesCartService] Anonymous profile set with status', profileRes.status)
	}
	...
	private static async _setMarketingData(
		...
		console.log('[SalesCartService] Marketing data set with status', marketingDataRes.status)
	}
	...
	private static async _setCustomData(
		...
		console.log('[SalesCartService] CustomData set with status', customDataRes.status)
	}
	...
	private static async _setMerchantContextData(
		...
		console.log('[SalesCartService] MerchantContextData set with status', merchantContextDataRes.status)
	}
	...
	static async getVendor(): Promise<{
		...
		const vendorData: Vendor | null = await Eitri.sharedStorage.getItemJson('vendorData')
		const salesAuthContext: SalesAuthContext | null = await Eitri.sharedStorage.getItemJson('salesAuthContext')
	...
	static async saveSalesAssisted(
		...
		try {
			...
			const { userContext } = await SalesCartService._salesAppLogin(baseUrl, account, vtexIdToken)

			console.warn('[SalesCartService] User context resolved:', userContext)

			const headers = SalesCartService._graphqlHeaders(vtexIdToken, account, userContext)
	 ...
	 */
	static async initSalesCart(vendor: Vendor, salesAuthContext: SalesAuthContext): Promise<OrderForm> {
		try {
			console.log('[SalesCartService] initSalesCart: Starting initialization...')
			...
			// Create orderForm
			const { orderFormId, userContext, orderFormData } = await SalesCartService._createOrderForm(
				baseUrl,
				account,
				sc,
			...
			// Check-in
			await SalesCartService._checkIn(baseUrl, account, sc, orderFormId, vendor.store_linked.pickupPoint)

			// Set session
			await SalesCartService._setSession(baseUrl, sc)

			// Login & resolve userContext
			const { userContext: resolvedUserContext } = await SalesCartService._salesAppLogin(
				baseUrl,
				account,
				vtexIdToken,
				userContext
			...
			// Set marketing data
			await SalesCartService._setMarketingData(baseUrl, account, sc, orderFormId, vendor.code, vendor.store)
			...
			await SalesCartService._setMerchantContextData(baseUrl, account, sc, orderFormId, vendor.code)
			...
			console.error('[SalesCartService] initSalesCart: Failed with error:', error)
			throw error
		}
	}
	 ...
	 */
	static async addItem(orderFormId: string, skuItem: SkuItem): Promise<OrderForm> {
		const { account, baseUrl } = await SalesCartService._getConfig()
		const orderItems = [
			{
				id: skuItem.itemId,
	...
	static async identifyCustomer(
		...
		console.log('[SalesCartService] identifyCustomer: Customer identified successfully')
	...
	static async getClientProfile(document: string): Promise<ClientProfile> {
		...
		console.log('[SalesCartService] getClientProfile: Profile fetched successfully')

		const profile = res.data as ClientProfile

		try {
			const identification = await SalesCartService._getClientProfileIdentification(baseUrl, headers, document)
			if (identification) {
				profile.identification = identification
				if (identification.user?.name) {
					profile.email = { data: identification.user.name }
				}
				console.log('[SalesCartService] getClientProfile: Identification fetched successfully')
			}
		} catch (e) {
			console.warn('[SalesCartService] getClientProfile: Failed to fetch client identification', e)
		}
	 ...
	 */
	static async removeItem(orderFormId: string, itemIndex: number): Promise<OrderForm> {
		const { baseUrl } = await SalesCartService._getConfig()
