...
export default function CheckoutMethod() {
	...
	const login = async () => {
		const result = await WakeService.customer.customerAuthenticatedLogin(userLogin.login, userLogin.pass)
	...
	const addressAssociate = async () => {
		const response = await WakeService.checkout.checkoutAddressAssociate(
			'eyJFbnRpdHkiOiJDdXN0b21lckFkZHJlc3MiLCJJZCI6NzQ4NDU2fQ=='
		)
		console.log('response >>', response)
	}
	...
	const setPaymentMethod = async () => {
		const response = await WakeService.checkout.checkoutSelectPaymentMethod(
			'eyJFbnRpdHkiOiJQYXltZW50TWV0aG9kIiwiSWQiOjkwNDN9'
		)
		console.log('response >>', response)
	}
	...
	const associateCheckoutPartner = async () => {
		try {
			const response = await WakeService.checkout.checkoutPartnerAssociate()
			console.log('response associateCheckoutPartner >>', JSON.stringify(response))
		} catch (e) {
			console.error('resetCheckout', e)
		}
		await getCheckout()
	...
	const disassociateCheckoutPartner = async () => {
		try {
			const response = await WakeService.checkout.checkoutPartnerDisassociate()
			console.log('response disassociateCheckoutPartner >>', JSON.stringify(response))
		} catch (e) {
			console.error('resetCheckout', e)
		}
		await getCheckout()
		...
		<Window
			...
			<View margin='large'>
				<View direction='column'>
					<Text>Checkout id: {fullCart?.checkoutId}</Text>
					<Text>Total de Produtos: {cart.quantity || '0'}</Text>
					<Text>Credito: {fullCart?.customer?.checkingAccountBalance || '0'}</Text>
					<Text>Credito associado: {fullCart?.checkingAccountValue || '0'}</Text>
				</View>
