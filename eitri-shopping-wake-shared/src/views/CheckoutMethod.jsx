...
export default function CheckoutMethod() {
  ...
  const addItemCart = async () => {
    try {
      ...
      const _fullCart = await WakeService.cart.addItems([
        { productVariantId: 346119, quantity: 1 },
      ]);
  ...
  const setShippingQuotes = async () => {
    const response =
      await WakeService.checkout.checkoutSelectShippingQuote(shippingQuote);
    console.log("response >>", response);
  };
  ...
  const setInstallments = async () => {
    const response = await WakeService.checkout.checkoutSelectInstallment(
      "7c3c7576-9aa3-48c0-8192-955e25500aea",
      3,
    );
    console.log("response >>", response);
  };
  ...
  const checkoutAddCoupon = async () => {
    const response =
      await WakeService.checkout.checkoutAddCoupon("PROMOAPP2025");
    console.log("response >>", response);
  };
  ...
  const useCashback = async () => {
    try {
      const response = await WakeService.checkout.checkoutUseCheckingAccount(
        fullCart.checkoutId,
      );
  ...
  const resetCheckout = async () => {
    try {
      const response = await WakeService.checkout.checkoutReset(
        fullCart.checkoutId,
      );
