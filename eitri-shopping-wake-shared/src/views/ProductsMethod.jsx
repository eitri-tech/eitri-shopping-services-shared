...
export default function ProductsMethod() {
  ...
  const getAllByCategory = async (cursor) => {
    ...
    resetView();
    try {
      const res = await WakeService.product.findAllByCategory(
        categoryId,
        cursor,
      );
      setProducts(res);
  ...
  const notifyMe = async () => {
    setLoading(true);
    try {
      const res = await WakeService.product.restockAlert(
        "teste",
        "teste.eitri@calindra.com.br",
        336903,
      );
        ...
        <>
          ...
          >
            <Button
              wide
              color="background-color"
              onPress={() => getProduct(productId)}
              label={`Buscar produto ${productId}`}
            />
          </View>
          <View
            padding="large"
          ...
          >
            <Button
              wide
              color="background-color"
              onPress={() => getAllByCategory("")}
              label={`Buscar na Categoria ${categoryId}`}
            />
          </View>
          <View
            padding="large"
          ...
          >
            <Button
              wide
              color="background-color"
              onPress={() => findByTerm(searchTerm, "")}
              label={`Buscar pelo termo: ${searchTerm}`}
            />
          </View>
          <View
            padding="large"
            ...
            <View marginTop="large" direction="column" gap={10} width="100%">
              {products.map((product, idx) => (
                <Text
                  padding="small"
                  key={idx}
                >{`${product.node.variantName} (${product.node.productId})`}</Text>
              ))}
              ...
              >
                <Touchable
                  onPress={() =>
                    getProductList(products[products.length - 1].cursor)
                  }
                >
                  <Text backgroundColor="positive-700" padding="small">
                    Next
                  </Text>
                </Touchable>
              </View>
