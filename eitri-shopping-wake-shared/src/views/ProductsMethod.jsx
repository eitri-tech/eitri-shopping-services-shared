...
export default function ProductsMethod() {
	...
	const getProductRecommendation = async productId => {
		setLoading(true)
		resetView()
		try {
			const res = await WakeService.product.productRecommendations(192228)
			console.log(res)
		} catch (e) {
			console.error(e)
		}
					...
					{products.length > 0 && (
						<View
							...
							{products.map((product, idx) => (
								<Text
									padding='small'
									key={idx}>{`${product.node.variantName} (${product.node.productId})`}</Text>
							))}
