...
export default class ProductService {
	...
	static async productRecommendations(
		...
		const query = `query {
			productRecommendations(productId: ${Number(productId)} algorithm: ${algorithm} sortDirection: ${sortDirection} sortKey: ${sortKey} quantity: ${quantity}) {
				${ProductService.defaultProductView}
			}
		}`
		...
		// console.log('query >>>>>>', query)
		const response = await GraphqlService.query(query)
		let products = response.productRecommendations || []
		// console.log('response >>>>>>', response)
		return products
	}
}
