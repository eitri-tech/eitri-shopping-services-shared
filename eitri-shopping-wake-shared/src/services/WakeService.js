...
export default class WakeService {
  ...
  static configure = async (remoteConfig) => {
    ...
    if (
      providerInfo.cartHost &&
      !providerInfo.cartHost.startsWith("https://")
    ) {
  ...
  static tryAutoConfigure = async (overwrites) => {
    ...
    try {
      ...
      console.error(
        "[SHARED] Error autoConfigure ",
        remoteConfig.ecommerceProvider,
        error,
      );
      throw error;
    }
    ...
    console.log(
      `[SHARED] App WAKE ${WakeService.configs.account} configurado com sucesso`,
    );
